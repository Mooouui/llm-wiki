...
- **Raw 层不可变**：`raw/` 目录是"真理之源"，LLM 只读不写。这保证你始终可以追溯到原始数据进行验证 ([[LLM Wiki]])
- **Wiki 层交给 LLM**：`wiki/` 层由 LLM 完全控制——你读它，LLM 写它。不要手动编辑 Wiki 页面，否则会破坏 LLM 的维护一致性 ([[LLM Wiki]])
- **Schema 层持续演化**：`CLAUDE.md` 是让人和 LLM 共同维护的"控制协议"。发现什么好用、什么不好用就立刻更新它 ([[Schema]])

### 2. Ingest 实践：一次一个，全程参与

- [[Andrej Karpathy]] 个人偏好一次摄入一个来源，全程参与——阅读摘要、检查更新、引导 LLM 该强调什么
...
- 这形成复利循环：来源被摄入 → 提问产生新洞察 → 洞察归档为新页面 → Wiki 从外部来源和内部探索双向增长 ([[LLM Wiki]])
...
| 规模期 | 100+ | 引入 [[Tobi Lutke]] 的 qmd（BM25 + 向量 + LLM 重排序） |

### 6. 最重要的实践：混合方案（地图 ≠ 领地）

[[Extended Brain]] 借助 [[Niklas Luhmann]] 的 [[Zettelkasten]] 和 [[Hormesis|认知摩擦]] 概念提出了最有深度的警告：LLM 写出流畅的综合文章，没有挣扎、没有语言暴露理解缺口的时刻。读这些文章的人可能觉得自己理解了主题，但没做过建构理解的认知工作。
